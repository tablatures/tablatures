<script lang="ts">
    ...
    $: if (api) { api.metronomeVolume = metronome }
...
<div class="m-5">
    ...
    <div class="sticky top-0 text-stone-500 px-1 z-[1001]">
        <div class="flex inline-block bg-light">
            ...
            <label class="flex overflow-hidden transition-all max-w-[30px] hover:min-w-[170px]" title="Manage playback speed">
                <button>
                    <i class="material-icons !text-2xl p-1">speed</i>
                </button>
                <input bind:value={speed} type="range" min="0.1" max="2" step="0.1" class="input-range w-[100px]">
                <span class="text-xs pt-3 pl-1">{Math.round(speed * 100)}%</span>
            </label>
            
            <button on:click={clickLooping} class="{api?.isLooping  ? 'text-secondary' : ''}" title="Loop the playback">
                <i class="material-icons !text-2xl p-1">restart_alt</i>
            </button>

            <div class="my-[5px] mx-1 border-r-[1px]" />

            <label class="flex overflow-hidden transition-all max-w-[30px] hover:min-w-[170px]" title="Manage playback volume">
                ...
                <input bind:value={volume} type="range" min="0" max="2" step="0.1" class="input-range w-[100px]">
                <span class="text-xs pt-3 pl-1">{Math.round(volume * 100)}%</span>   
            </label>

            <label class="flex overflow-hidden transition-all max-w-[30px] hover:min-w-[170px]" title="Manage metronome volume">
                ...
                <input bind:value={metronome} type="range" min="0" max="2" step="0.1" class="input-range w-[100px]">
        ...
        <div class="{bindDuration ? 'h-[5px] text-transparent' : 'h-[14px] text-light' } hover:h-[14px] hover:text-light absolute w-full text-xs overflow-hidden" >
            <input type="range" class="loading-bar" min="0" max="100" bind:this={range} bind:value={progress} on:input={progressInput} on:click={progressClick} on:change={progressChange}>
