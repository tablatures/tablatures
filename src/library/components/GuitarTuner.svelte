<script lang="ts">
	...
	import { tunerStore, GUITAR_STRINGS } from '../utils/tuner';
	...
	$: closestStringIndex = state.frequency
		? GUITAR_STRINGS.reduce(
				(closest, gs, i) => {
					const diff = Math.abs(gs.frequency - (state.frequency ?? 0));
	...
	function drawHistory() {
		...
		const x5l = cToX(-5);
		const x5r = cToX(5);
		ctx.fillStyle = isDark ? 'rgba(16,185,129,0.04)' : 'rgba(16,185,129,0.03)';
...
{#if open}
	...
	>
		<div
			class="pointer-events-auto w-full h-full sm:h-auto sm:max-h-[85vh] sm:max-w-[640px] bg-white dark:bg-neutral-900 sm:border border-neutral-200 dark:border-neutral-700 shadow-2xl sm:rounded-2xl overflow-hidden flex flex-col"
			transition:fade={{ duration: 100 }}
		>
			<!-- Header bar -->
			<div
			...
			>
				<div class="flex items-center gap-2.5 text-neutral-700 dark:text-neutral-300">
					<i class="material-icons !text-xl text-violet-500">network_check</i>
			...
			<div class="px-2 flex-1 sm:flex-none flex flex-col" style="min-height: 280px">
				...
				<div class="relative w-full h-8 flex items-center">
					<!-- Track -->
					<div
						class="absolute h-1.5 bg-neutral-200 dark:bg-neutral-700 rounded-full"
						style="left: {PAD}%; right: {100 - (PAD + RANGE)}%"
					/>

					<!-- In-tune zone -->
					<div
						class="absolute h-1.5 rounded-full bg-emerald-300/50 dark:bg-emerald-800/40"
					...
					{#each [-50, -25, 0, 25, 50] as c}
						...
						<div
							class="absolute top-1/2 -translate-y-1/2 w-0.5 rounded-full
								{c === 0 ? 'h-5 bg-neutral-400 dark:bg-neutral-500' : 'h-3 bg-neutral-300 dark:bg-neutral-600'}"
							style="left: {pos}%"
						/>
					{/each}

					{#if state.active}
						<!-- Layer 1: black outline shapes (background) -->
						{#if trailWidth > 0.3}
							<div
								class="absolute top-1/2 -translate-y-1/2 bg-black/30 dark:bg-black/60 needle-taper"
						...
						<!-- Layer 2: white fill shapes (foreground, no outline) -->
						{#if trailWidth > 0.3}
							<div
								class="absolute top-1/2 -translate-y-1/2 bg-white needle-taper"
						...
						{/if}
						<div
							class="absolute top-1/2 w-2.5 h-2.5 rounded-full bg-white"
							style="left: {trailedPos}%; transform: translate(-50%, -50%); z-index: 2"
						/>
						<div
							class="absolute top-1/2 w-3 h-3 rounded-full bg-white"
			...
			<div class="px-4 sm:px-5 pb-4 sm:pb-5 pt-3 sm:pt-4 flex flex-col items-center gap-3 sm:gap-4">
				...
				<div class="flex items-center gap-1.5 sm:gap-2 flex-wrap justify-center">
					{#each GUITAR_STRINGS as gs, i}
						<button
							on:click={() => tunerStore.setTargetString(i)}
							class="w-10 h-10 sm:w-11 sm:h-11 rounded-xl text-xs font-mono font-semibold transition-all duration-150 active:scale-95
					...
					<button
						...
						class="h-10 sm:h-11 px-3 sm:px-4 rounded-xl text-xs font-semibold transition-all duration-150 active:scale-95
...
<style>
	...
	/* Background outline layer — slightly larger black shape behind the white fill */
	.needle-outline-bg {
		box-shadow:
